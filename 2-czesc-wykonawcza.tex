...
ultradźwiękowy, mikrokontroler oraz \ac{acr_tcxo}, o wyborze którego zadecydowały potencjalne wahania temperatury.
...
\ac{acr_tcxo} według noty aplikacyjnej \cite{ST_AN5646} jest droższym, lecz lepszym rozwiązaniem w~porównaniu do standardowego oscylatora kwarcowego -- szczególnie w~kontekście dokładności oscylacji.
Dzięki \ac{acr_tcxo} można osiągnąć mniejszy dryft (ang. \textit {drift}). Zaletą tego rozwiązania jest możliwość pracy w~węższym paśmie częstotliwości, co wiąże się ze zwiększeniem zasięgu transmisji. Mniejszy dryft powoduje, że 
...
integrujące mikrokontroler i~moduł radiowy obsługujący system LoRa (ang. \textit{Long Range}), który jest obok SigFox najbardziej energooszczędnym systemem komunikacji (rysunek \ref{fig:zestawienie_interfejsow}).
...
\begin{figure}[H]
    \centering \includegraphics[width=1\textwidth]{zestawienie_rodziny_wl_oprac.png}
    \caption{Rodzina STM32WL od \textit{STMicroelectronics}.\\[2mm]
    \small Źródło: Opracowanie własne (tłumaczenie) na podstawie~\cite{FamilySTM32WL}.    
    }
    \label{fig:zestawienie_rodziny_wl}
\end{figure}

W projekcie przyjęto komunikację \textit{LoRa~P2P} (ang. \textit{point--to--point}), umożliwiającą bezpośrednią wymianę danych pomiędzy dwoma urządzeniami bez udziału infrastruktury
sieciowej. Jednocześnie przewidziano możliwość zastosowania standardu LoRaWAN (ang. \textit{Long Range Wide Area Network}) w przypadku przyszłej rozbudowy systemu lub konieczności zwiększenia jego zasięgu
...
Dodatkową zaletą jest to, że w~ramach serii WLE5 w~tej samej obudowie znajdują się jeszcze dwa układy, które posiadają kolejno 128 KB/48 KB i~256 KB/64 KB pamięci flash\footnote{Pamięć flash w 
mikrokontrolerach to coś w rodzaju „twardego dysku na mikroskalę” — trzyma program nawet bez zasilania, ale każda komórka wytrzymuje tylko ograniczoną liczbę cykli kasowania. Dlatego dobry firmware
traktuje flash z szacunkiem: zapisuje rzadko, rotuje sektory i dba, by nie zużyć najcenniejszego zasobu MCU szybciej niż potrzeba.} i~RAM\footnote{RAM w mikrokontrolerze to najszybsze, ale i
najbardziej kapryśne miejsce pracy programu — znika po odcięciu zasilania, więc każda awaria potrafi wyzerować całą „krótką pamięć” urządzenia w jednej chwili. Dlatego systemy krytyczne często
trzymają ważne dane w rejestrach lub backup-RAM, a RAM traktują jak przestrzeń roboczą jednorazowego użytku.}$^{, }$\footnote{W świecie MCU RAM bywa tak ciasny, że programista czasem czuje się jak
zegarmistrz — każdy bajt jest na wagę złota, a nieoptymalna tablica potrafi „wysadzić” cały projekt. To właśnie dlatego w embedded tak popularne są techniki oszczędzania pamięci: statyczne bufory,
unikanie alokacji dynamicznej i przechowywanie stałych w flashu zamiast w RAM-ie.} ,
co może być przydatne w~przypadku zwiększonego zapotrzebowania pamięci w~przyszłości. Seria WL55została odrzucona, ponieważ posiada tylko jedną wersję pamięciową.
      ...
      \begin{figure}[!h]
        ...
        \begin{table}[H] \centering
                ...
                \begin{tabular}{r c c}
                    ...
                    \textbf{moduł pomiarowy}                                                & $46,5 - 33,6j$  &               $65,1 - 13,5j$                  \\  
                    \textbf{stacja bazowa}                                                  & $44,2 - 36,7j$  &               $64,9 - 8,8j$                   \\ 
                    \hline\hline
                \end{tabular}
        \end{table}

        % dopisane
            Istnieje pojęcie takie jak \ac{acr_wdt}. Odpowiednikiem \ac{acr_wdt} jest współczynnika odbicia $\mathit{\Gamma}$. W~torach radiowych kluczowym etapem projektowania jest uzyskanie dopasowania impedancji pomiędzy wyjściem nadajnika $Z_L$ a~impedancją anteny $Z_O$.
            Niedopasowanie powoduje wzrost $\mathit{\Gamma}$, a~tym samym zwiększenie mocy odbitej oraz pogorszenie efektywności promieniowania. Dla częstotliwości $f = 868,0\,\text{MHz}$
            przyjmuje się, że dopasowanie jest akceptowalne, gdy moduł współczynnika odbicia spełnia warunek  $\mathit{\Gamma} < 0,2$. 
        % dopisane
        Na podstawie zebranych wyników przeprowadzono obliczenia parametrów takich jak $\mathit{SWR}$ (ang. \textit{standing wave ratio}) oraz $\mathit{\Gamma}$ do łatwiejszego oszacowania poziomu dopasowania toru.
        
        \begin{align}
            \label{eq:gamma}
            \mathit{\Gamma} &= \frac{Z_L - Z_0}{Z_L + Z_0} = \frac{(46,5 - 33,6j)\,\Omega - 50\,\Omega}{(46,5 - 33,6j)\,\Omega + 50\,\Omega}  \\
            \mathit{\Gamma} &= \frac{(-3,5 - 33,6j)\,\Omega}{(96,5 - 33,6j)\,\Omega} = 0,075 - 0,322j  
        \end{align}
        
        Stąd moduł wyrażenia:
        \begin{align}
            \mathit{\Gamma} &= \sqrt{(0,075)^2 + (-0,322)^2} = 0,331
        \end{align}
       
        Na tej podstawie SWR: 
        \begin{align}
            \mathit{SWR} &= \frac{1 + |\Gamma|}{1 - |\Gamma|} = \frac{1 + 0,331}{1 - 0,331} = 1,99 
        \end{align}

        Moc odbita od anteny wyraża się wzorem $\mathit{P_{\text{odbita}}} = |\Gamma|^2 \cdot 100\,\% =  0,331^2 \cdot 100\,\% = 10,9\,\%.$
        ...
        Przyczyną był prawdopodobnie fakt przeprowadzania testów na \textit{zaszumionych} obszarach miejskich oraz trzymanie urządzeń nisko nad ziemią. Przeprowadzono test w~skrajnie niekorzystnych warunkach, co oznacza
